...
export default {
	...
	data () {
		return {
			...
			quantity: null,
			deliveryId: null,
			deliveryOptions: [{id: 'pickup', name: 'Самовывоз из Санкт-Петербурга'}, {id: 'courier', name: 'Доставка курьером по России'}],
			foxCities: [],
			isCity: false,
			city: {
				city_name: '',
	...
	methods: {
		...
		calcDeliveryPrice() {
			...
			if (!this.hasError) {
				...
				api.getFoxPrice(this.city.guid).then(
					res => {
		...
		checkOrderValid() {
			...
			if (this.isMerch) {
				...
				if (this.quantity > (this.priceId ? this.prices.find(p => p.id === this.priceId).available_quantity : 0)) {
					this.hasError = true
				}
			}
		...
		makeOrder() {
			...
			api.makeProguctOrder(order, this.$store.state.user.token).then(res => {
		...
		makePayment(order_id) {
			...
			api.makeProguctPayment(data, this.$store.state.user.token).then(res => {
			}, rej => {
			})
		},
		errorHandler(errors) {
